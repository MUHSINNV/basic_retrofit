...
public class Viewdata extends AppCompatActivity implements ListView.OnItemClickListener {
    ...
    private List<Viewmodeclass> books;
    ...
    private void getBooks(){
        ...
        ViewApi api = adapter.create(ViewApi.class);

        //Defining the method
        api.getBooks(new Callback<List<Viewmodeclass>>() {
            @Override
            public void success(List<Viewmodeclass> list, Response response) {
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        Viewmodeclass book = books.get(position);

        //Adding book details to intent
        intent.putExtra(KEY_BOOK_ID,book.getId());
        intent.putExtra(KEY_BOOK_NAME,book.getName());
        intent.putExtra(KEY_BOOK_PRICE,book.getUsername());
        intent.putExtra(KEY_BOOK_STOCK,book.getPassword());
